...
/**
 * HeroSection — Section 1 of the portfolio.
 *
 * Design language inspired by the Sui website: pure-black cinematic canvas,
 * sharp white typography, a single controlled blue accent (#298DFF), layered
 * floating "system" cards, and restrained product-grade motion.
 *
 * Framework-agnostic React component (works in Vite or Next.js App Router).
 * If used in Next.js App Router, add "use client" at the top of the file.
 */
...
type CardItem = {
  ...
  /** desktop offset for the layered, depth-based arrangement */
  className: string;
  rotate: number;
  /** parallax depth factor — larger = moves more on scroll */
  depth: number;
  floatDelay: number;
};
...
function TiltCard({
  ...
  // scroll parallax — each card drifts at its own rate for layered depth
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-start gap-3 pl-3">
          <div
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] text-[#298DFF]"
          >
            {card.icon}
          </div>
...
function HeroBackground({
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      {/* radial blue glow behind the card cluster */}
      <motion.div
...
export default function HeroSection({
  ...
  // scroll progress mapped 0 -> 1 across the hero
  ...
  // left column drifts up slightly slower than the right composition
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-7xl flex-col justify-center px-6 pt-28 pb-20 sm:px-8 lg:px-12">
        <div className="grid items-center gap-16 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          {/* mobile-only compact card stack */}
          <motion.div
            variants={cardCluster}
            initial="hidden"
            animate="show"
